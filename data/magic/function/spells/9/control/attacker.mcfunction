...
execute at @e[type=item_display,tag=manaChain,predicate=magic:match_chain] positioned ~-0.25 ~-0.25 ~-0.25 as @e[distance=..12,type=#magic:targets,predicate=!magic:match_id,dx=0] positioned ~-0.5 ~-0.5 ~-0.5 if entity @s[dx=0] unless score @s magic.iframe matches 1.. run damage @s 15 player_attack by @n[type=#magic:magic_user,tag=chainOwner]
