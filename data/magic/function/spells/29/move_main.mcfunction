...
execute if score @s magic.temp matches 400.. run function magic:spells/29/start_explode

# damage
tag @n[type=#magic:magic_user,predicate=magic:match_id] add thisPlayer
tag @s add thisOrb
execute as @e[type=#magic:targets,distance=..3.3,predicate=!magic:match_id] unless score @s magic.iframe matches 1.. at @s run damage @s 15 player_attack by @n[type=#magic:magic_user,tag=thisPlayer]
