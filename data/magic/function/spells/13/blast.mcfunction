...
playsound minecraft:item.totem.use master @a ~ ~ ~ 8 0.8
playsound minecraft:entity.warden.sonic_boom master @a ~ ~ ~ 8 0.3
playsound minecraft:entity.warden.sonic_boom master @a ~ ~ ~ 8 0.8
playsound minecraft:entity.lightning_bolt.thunder master @a ~ ~ ~ 8 0.5

tag @n[type=#magic:magic_user,predicate=magic:match_id] add thisPlayer
execute as @e[type=#magic:targets,distance=..35] unless score @s magic.iframe matches 1.. run damage @s 500 magic:ignore_cooldown by @n[type=#magic:magic_user,tag=thisPlayer]
scoreboard players add @e[type=#magic:targets,distance=..35] magic.stagger 120
